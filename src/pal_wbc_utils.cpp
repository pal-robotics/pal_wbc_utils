...
namespace pal {
std::string
generateTaskDescription(const property_bag::PropertyBag properties) {
  std::stringstream ss;
...
WBCServiceHelper::WBCServiceHelper(ros::NodeHandle &nh, const std::string &ns)
    : nh_(nh), ns_(ns) {
  bool persistent =
      false; // Not persistent, because when we enable WBC the services are
...
bool WBCServiceHelper::check_if_services_are_ready(
  ...
  if ((!ready_1) || (!ready_2) || (!ready_3) || (!ready_4) || (!ready_5))
    return false;
...
std::string
WBCServiceHelper::orderToString(pal_wbc_msgs::Order::_order_type order) {
  switch (order) {
...
bool WBCServiceHelper::pushTask(const property_bag::PropertyBag &properties,
  ...
  srv.request.push_task_params.params =
      generateTaskDescription(task_properties);
  ...
  if (push_task_srv_.call(srv)) {
    ROS_INFO_STREAM("Succesfully pushed task:" << task_id << " "
                                               << orderToString(order) << " "
                                               << respect_task_id);
...
bool WBCServiceHelper::pushPopTask(
    const property_bag::PropertyBag &properties,
    const std::string &push_task_id,
    const pal_wbc_msgs::Order::_order_type &order,
    const std::string &push_respect_task_id, const std::string &pop_task_id) {
  if (!check_if_services_are_ready(ros::Duration(10.0)))
    return false;
  ...
  if (push_pop_task_srv_.call(srv)) {
    ROS_INFO_STREAM("Succesfully pushed task "
                    << push_task_id << " and popped tasks " << pop_task_id);
    printStackDescription();
    return true;
  } else {
...
pal_wbc_msgs::TaskError
WBCServiceHelper::getTaskError(const std::string &task_id) {
  if (!check_if_services_are_ready(ros::Duration(10.0)))
...
bool WBCServiceHelper::waitForConvergence(
    const std::string &task_id, double eps, const ros::Duration &timeout,
    double log_throttle, const ros::Duration &check_error_rate,
  ...
  do {
    ...
    while (ros::ok() &&
           (ros::Time::now() < next_error_check)) // Inside this loop we
                                                  // check for peremptions
                                                  // at 100Hz
    {
      if (abort_condition()) {
        ROS_WARN_STREAM("Wait for " << task_id
                                    << " preempted, remember to "
    ...
    ROS_INFO_STREAM_THROTTLE(log_throttle, "Task: " << task_id
                                                    << " error norm is: "
                                                    << task_error.error_norm);
    if (ros::Time::now() > timeout_time) {
      ROS_ERROR_STREAM("Task did not converge");
      return false;
    }
  } while (ros::ok() && (task_error.error_norm > eps));
  ROS_INFO_STREAM("FINISH Task: " << task_id << " error norm is: "
                                  << task_error.error_norm);
